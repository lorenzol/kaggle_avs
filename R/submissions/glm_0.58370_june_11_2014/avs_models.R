...
train.batch <- train[training.rows, c(3:57) ]
test.batch <- train[-training.rows, c(3:57) ]
...
#features by column number
features1 <- paste(names(train[,6:30]), sep="+")

#features by column name
features2 <- paste("tot_dis_items_p", 
                  "tot_n_depts", 
                  sep="+")

fmla <- as.formula(paste("repeater ~ ", paste(features1, collapse= "+")))
...
glmnetGrid <- expand.grid(.alpha = 0.002, 
                          .lambda = 0.001)
